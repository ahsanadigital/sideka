...
const enableInputs = () => {
    ...
    $('[id^="user-select_"]')
        ...
        .removeAttr("disabled")
        .trigger("change");
    $('[id^="user-select_"]').next(".select2-container").show();
};
...
/**
 * Initializes the Quill editor with dynamic options and target.
 *
 * @param {string} target - The selector of the target element.
 * @param {object} [options={}] - The options for the Quill editor.
 * @returns {Quill} The initialized Quill editor instance.
 */
function initQuillEditor(target, options = {}) {
    ...
    const quill = new Quill(target, quillOptions);
